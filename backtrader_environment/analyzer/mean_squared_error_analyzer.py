...
class MeanSquaredErrorAnalyzer(GeneralAnalyzer):
    def __init__(self):
        super().__init__()
    ...
    def stop(self):
        for d in self.strategy.datas:
            if self.num_of_data_points[d] < 3:
                ...
                self.mean_squared_error[d._name] = self.current_error_sum[d] / self.num_of_data_points[d]

    def get_analysis(self):
        super_analysis = super().get_analysis()
        analysis = {}
        analysis["general"] = super_analysis
        analysis["mean_squared_error"] = self.mean_squared_error
        return analysis
